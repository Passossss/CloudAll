...
console.log('🧪 Executando Testes do User Service\n');
...
if (fs.existsSync(integrationTestDir)) {
  ...
  if (integrationTests.length > 0) {
    console.log(`Encontrados ${integrationTests.length} teste(s) de integração`);
    integrationTests.forEach(test => {
      console.log(`  - ${test}`);
    });
    console.log('ℹ️  Testes de integração requerem banco de dados configurado');
  } else {
    console.log('⚠️  Nenhum teste de integração encontrado');
  }
} else {
  console.log('⚠️  Diretório de testes de integração não encontrado');
}
